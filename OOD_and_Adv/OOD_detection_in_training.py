...
def process(rootpath, preprocess=None):
    ...
    if "CIFAR-100" in rootpath:
        valset = datasets.CIFAR100(root='/slstore/tianfeng/data', train=True, download=True, transform=transform_test)

        valloader = torch.utils.data.DataLoader(valset, batch_size=100, shuffle=False, pin_memory=True,
                                                num_workers=0)
        num_classes = 100
    elif "CIFAR-10" in rootpath:
        valset = datasets.CIFAR10(root='/slstore/tianfeng/data', train=True, download=True, transform=transform_test)
...
rootpaths = [
        "/slstore/tianfeng/IB_BNN/results/CIFAR-10_vgg/baseline/new_seed42_l0.01_epoch200_datasetCIFAR-10_modelvgg_samplings1_ratio0.0_inits-1.8971199848858813/",
    ]
preds0, labels0, Has0, Hes0, ib0 = process(rootpaths[0])
...
index = (Has0 + Hes0).argmax()
hardIB = alldata[:, index, :]
meanIB = alldata.mean(dim=1)

n_layers = [3, 7, 10, 14, 17, 20, 24, 27, 30, 34, 37, 40]
for i in range(12):
    plt.plot(hardIB[:, i])
    plt.plot(meanIB[:, i])
    plt.legend(["hard sample", "average"])
    plt.title(f"Layer {n_layers[i]}")
    plt.xlabel("epoch")
    plt.ylabel("IB")
    plt.show()